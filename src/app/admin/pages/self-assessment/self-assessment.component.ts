...
export class SelfAssessmentComponent {
  ...
  assessmentCriteria1: AssessmentCriteria[][] = new Array<Array<AssessmentCriteria>>();
  ...
  addAssessmentToList() {
    this.createdSelfCriticism.assessmentCriterias.push(new AssessmentCriteria());
  ...
  selectChangeAssessmentCriteriaGroup(selectedAssGroup: AssessmentCriteriaGroup, i: number) {
    if (!this.createdSelfCriticism.assessmentCriterias.length) {
      return;
    }

    console.log(selectedAssGroup);
    this.assessmentCriteria1[i] = this.assessmentCriteria.filter(item => item.assessmentCriteriaGroupId == selectedAssGroup.id);
    this.createdSelfCriticism.assessmentCriterias[i] = this.assessmentCriteria.filter(item => item.assessmentCriteriaGroupId == selectedAssGroup.id)[0];
